"""Step 6 — render the draft review video.

Combines:
  - Stage 3 voiceover + manifest (anchored ranges + audio timing)
  - Stage 4 subtitle manifest (timed subtitle cues)
  - Stage 5 clips, keyframes, clip_manifest (per-range hero clips)
  - Stage 0 visual_segments (shot boundaries for shot-aware smart-trim)

Writes: tmp/work/<movie_slug>/stage6/review.mp4
        tmp/work/<movie_slug>/stage6/segments/segment_NNN.{mp4,mp3}
        tmp/work/<movie_slug>/stage6/edit_manifest.json
"""
...
from app.pipeline.stage6_render_video import main as stage6_main
...
def run() -> int:
    ...
    required = [
        ...
        ("subtitle manifest", paths.stage4_subtitle_manifest),
        ("clips dir", paths.stage5_clips_dir),
        ("keyframes dir", paths.stage5_keyframes_dir),
        ("clip manifest", paths.stage5_clip_manifest),
    ]
    for label, p in required:
        if not p.exists():
            return fail(f"{label} missing: {p}")

    banner(f"Stage 6 — render draft review video for {cfg['common']['movie_title']}")
    print(f"voiceover  : {paths.stage3_voiceover}")
    print(f"clips dir  : {paths.stage5_clips_dir}")
    print(f"output     : {paths.stage6_review_video}")

    argv = [
        "--manifest", str(paths.stage3_manifest),
        "--voiceover", str(paths.stage3_voiceover),
        "--subtitle-manifest", str(paths.stage4_subtitle_manifest),
        "--clips-dir", str(paths.stage5_clips_dir),
        "--keyframes-dir", str(paths.stage5_keyframes_dir),
        "--clip-manifest", str(paths.stage5_clip_manifest),
        "--output", str(paths.stage6_review_video),
    ]
