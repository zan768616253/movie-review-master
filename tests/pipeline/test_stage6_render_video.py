...
from app.pipeline.stage6_render_video import (
...
def test_plan_smart_trim_keeps_exact_multi_range_when_no_trim_needed() -> None:
    ranges = [(10.0, 14.0), (20.0, 22.0)]
    kept, kind = plan_smart_trim(ranges, [[], []], audio_duration_s=6.0)
    assert kept == ranges
    assert kind == "exact"


def test_plan_smart_trim_spreads_excess_across_multiple_ranges() -> None:
    # Two ranges: [10–15] (5s) and [20–24] (4s); audio 6s; excess 3s.
    # Both ranges can still keep a readable hold after proportional trim,
    # so Stage 5 should preserve both rather than collapsing to one.
    ranges = [(10.0, 15.0), (20.0, 24.0)]
    shots = [[], []]
    kept, kind = plan_smart_trim(ranges, shots, audio_duration_s=6.0)
    assert kept[0] == (10.0, 15.0 - 3.0 * (3.5 / 6.0))
    assert kept[1] == (20.0, 24.0 - 3.0 * (2.5 / 6.0))
    assert len(kept) == 2
    assert kind == "mid-shot-spread"


def test_plan_smart_trim_drops_twitchy_tail_when_small_fill_can_replace_it() -> None:
    ranges = [(10.0, 14.0), (20.0, 22.0)]
    shots = [[], []]
    kept, kind = plan_smart_trim(ranges, shots, audio_duration_s=5.0)
    assert kept == [(10.0, 14.0)]
    assert kind == "extension-needed"


def test_plan_smart_trim_keeps_twitchy_tail_when_drop_would_need_large_fill() -> None:
    ranges = [(10.0, 14.0), (20.0, 22.0)]
    shots = [[], []]
    kept, kind = plan_smart_trim(ranges, shots, audio_duration_s=5.5)
    assert len(kept) == 2
    assert (kept[-1][1] - kept[-1][0]) < 2.5
    assert kind == "mid-shot-spread"
...
def test_detect_black_intervals_parses_ffmpeg_blackdetect_output(
    ...
    monkeypatch.setattr(
        "app.pipeline.stage6_render_video.subprocess.run", fake_run
    )
...
def test_detect_black_intervals_returns_empty_when_no_match(
    ...
    monkeypatch.setattr(
        "app.pipeline.stage6_render_video.subprocess.run",
...
def test_write_subtitle_script_creates_styled_bottom_center_dialogue(tmp_path: Path) -> None:
    ...
    assert "Dialogue: 0,0:00:00.00,0:00:02.34,ReviewCaption,,0,0,0,,First line\\NSecond line" in payload
...
def test_main_renders_anchored_chunk_and_closing_keyframe(tmp_path: Path, monkeypatch) -> None:
    ...
    output_path = tmp_path / "stage6" / "review.mp4"
    ...
    monkeypatch.setattr("app.pipeline.stage6_render_video.resolve_encoder", lambda: "fake-codec")
    monkeypatch.setattr("app.pipeline.stage6_render_video.render_excerpt", fake_render_excerpt)
    monkeypatch.setattr("app.pipeline.stage6_render_video.render_stillframe_segment", fake_render_stillframe_segment)
    monkeypatch.setattr("app.pipeline.stage6_render_video.concat_segments", fake_concat_segments)
    monkeypatch.setattr("app.pipeline.stage6_render_video.split_voiceover_to_segment", fake_split_voiceover_to_segment)
    monkeypatch.setattr("app.pipeline.stage6_render_video.write_subtitle_script", fake_write_subtitle_script)
    monkeypatch.setattr("app.pipeline.stage6_render_video.burn_subtitles", fake_burn_subtitles)
    monkeypatch.setattr("app.pipeline.stage6_render_video.mux_audio", fake_mux_audio)
    monkeypatch.setattr(
        "app.pipeline.stage6_render_video.detect_black_intervals",
        lambda *a, **kw: [],
    )
