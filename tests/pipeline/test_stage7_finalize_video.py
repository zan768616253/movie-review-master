...
from app.pipeline.stage7_finalize_video import build_ffmpeg_command, main


def test_build_ffmpeg_command_maps_stage5_video_and_stage3_audio() -> None:
    command = build_ffmpeg_command(
        Path("/tmp/stage6/review.mp4"),
        Path("/tmp/stage3/voiceover.mp3"),
        Path("/tmp/stage7/final_video.mp4"),
    )
...
def test_main_rejects_missing_review_video(tmp_path: Path, capsys) -> None:
    ...
    result = main([
        ...
        str(voiceover),
        "--output",
        str(tmp_path / "stage7" / "final_video.mp4"),
    ])
...
def test_main_muxes_upload_video_and_writes_delivery_manifest(
    ...
    review_video = tmp_path / "stage6" / "review.mp4"
    ...
    voiceover.write_bytes(b"audio")
    output_path = tmp_path / "stage7" / "final_video.mp4"
    ...
    monkeypatch.setattr("app.pipeline.stage7_finalize_video.subprocess.run", fake_run)
