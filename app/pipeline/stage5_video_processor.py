"""Stage 5: extract per-anchor hero clips and keyframes from an anchored script.
...
Asymmetric pre/post handles give Stage 6 runway to absorb TTS-pacing
...
Closing chunks (no anchor) are skipped entirely; Stage 6 falls back to
...
def write_clip_manifest(output_dir: Path, plans: list[AnchorClipPlan]) -> Path:
    """Write `clip_manifest.json` consumed by Stage 6.
