...
Reads the Stage 5 draft review (`review.mp4`) and the Stage 3 voiceover,
then remuxes the Stage 5 video stream with the Stage 3 narration track into
an MP4 with `+faststart`, ready for direct upload.

Outputs:

    <output_dir>/final_video.mp4        upload-ready master
    <output_dir>/delivery_manifest.json stage-6 delivery metadata
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="finalize-video",
        description="Mux the Stage 5 draft render with the Stage 3 narration track.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--review-video", type=Path, required=True,
                        help="Stage 5 draft render MP4.")
    ...
    parser.add_argument("--manifest-output", type=Path,
                        help="Optional override for the stage-6 delivery manifest path.")
    return parser
