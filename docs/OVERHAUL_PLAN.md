...
### Phase 8 — Shot-aware-range contract (sync fix)

**Trigger:** post-Phase-7 user feedback. The "prefer fewer longer holds"
guidance from earlier prompt iterations let the planner pick anchors of
20+ seconds spanning multiple internal beats. Audio narration runs at a
uniform pace; source video runs at the editor's pace. Inside long anchors
the two paces diverge — observed drift was 2–8s with audio leading the
video that was supposed to illustrate it. Per-chunk durations were
matched at the rendering layer; the drift was content-level, inside long
anchors that crossed shot cuts.

**Theory:** make every range visually atomic. A range that stays inside
ONE source shot is a single coherent beat by the editor's definition,
so narration cannot "race ahead" of the visual within a range. The
anchor remains the narrative unit; the range becomes the visual unit
(one shot). Multi-shot beats use multi-range anchors.

**Contract changes:**

- `MAX_ANCHOR_RANGE_DURATION_S` lowered 60 → 12 (each range = one shot,
  most shots are 1–8s, 12s accommodates legitimate long takes while
  catching planner typos in end timestamps).
- New `MAX_ANCHOR_TOTAL_DURATION_S = 12` cap on anchor total duration
  (sum of range durations). Bounds worst-case within-anchor drift.
- New validator check `range_shot_crossing` rejects any range whose
  window contains a shot cut (with ±0.3s float-safety tolerance).
  Powered by `build_shot_boundary_set(visual_segments)` which unions
  segment start/end timestamps with each segment's `shot_boundaries_s`.
- Stage 2 timeline format: `[visual:NNN]` event entries replaced by
  `[shot:NNN] HH:MM:SS --> HH:MM:SS (Xs) :: ...` per-shot entries,
  generated by `split_segment_into_shots(segment)`. Stage 0's per-segment
  `shot_boundaries_s` annotation drives the split. Each visual segment
  with N inner cuts expands into N+1 [shot:NNN] entries; segments with
  empty `shot_boundaries_s` stay as one [shot:NNN]. Sub-shots shorter
  than 0.5s (flickers) are dropped.
- Planner prompt rewritten: anchor ranges MUST come from `[shot:NNN]`
  lines (not `[srt:NNN]`); each range stays inside one shot; multi-shot
  beats use multi-range anchors with one range per shot; sub-shot trims
  allowed but cannot bridge a shot boundary.
- Anchor-count guidance updated for shot-aware bounds:
  `target_seconds / 12` (low) to `target_seconds / 6` (high). For a 12-min
  target: ~60–120 anchors instead of the previous 8–18.

**Files changed:**

- `app/pipeline/common/script_contract.py` — constants, `build_shot_boundary_set`, `_check_range_shot_alignment`, validator threading.
- `app/pipeline/stage2_generate_script.py` — `split_segment_into_shots`, `build_merged_timeline` rewrite, prompt rewrite (TTS Budget / Source Material / Authoring Algorithm / Hard Constraints / Final Reminder).
- `tmp/step_02_generate_script.py` — pass `shot_boundaries` into validator; updated fix-request constraint reminder.
- `tests/pipeline/test_script_contract.py` — 10 new tests for `anchor_too_long`, `range_shot_crossing`, `build_shot_boundary_set`, tolerance handling, `None`-skip behaviour.
- `tests/pipeline/test_stage2_generate_script.py` — 5 new tests for `split_segment_into_shots`, shot-timeline format, plus updated assertions on the planner prompt.

**Bad cases knowingly accepted:**

1. Action montages with 1–2s shots ⇒ many short anchors. The cut frequency
   matches the source's editing pace; audience perceives this as
   intentional rhythm rather than nervous editing.
2. Long static shots (30s+) get sub-trimmed to ≤12s of source coverage.
   Loses some take, but stays inside the anchor cap.
3. Imperfect ffmpeg scene detection — soft transitions (fade, dissolve)
   may not register; the validator can't reject what Stage 0 missed.
   Mitigated by visual_segment start/end being shot-snapped, not by VLM
   approximation.

**Acceptance criteria:** subjective sync check on a fresh JJK0 run.
Generate script with the new prompt; render; verify the audio-vs-visual
drift inside any single anchor stays under ~2s. Test target:
worst-case drift inside an anchor ≤ shot length × pace mismatch ≤ ~3s.
...
- 2026-04-30 — Phase 8 (shot-aware-range contract) shipped on `video-lag-fix` branch. Eric flagged 2–8s audio-vs-video drift inside long anchors after the "fewer longer holds" prompt change. Diagnosis: per-chunk durations matched, drift was content-level inside multi-event anchors. Fix: anchor ranges must stay inside one source shot; `MAX_ANCHOR_RANGE_DURATION_S` 60→12, new `MAX_ANCHOR_TOTAL_DURATION_S` 12, new validator check `range_shot_crossing` driven by `build_shot_boundary_set`, Stage 2 timeline emits `[shot:NNN]` entries split by `shot_boundaries_s`. Tests: 56/56 in `test_script_contract.py` + `test_stage2_generate_script.py` pass (15 new shot-aware tests). Other pipeline tests unchanged. 5 pre-existing failures in stage5/stage6 are unrelated to this PR (verified via `git stash`). Subjective JJK0 acceptance pending.
- 2026-04-30 — Phase 9 (sync-fix follow-on sweep). Audit during the Phase 8 verification turned up three problems: (1) `merge_segments` shifted segment.start/end by chunk offset but forgot to shift `shot_boundaries_s`, leaving 96% of inner cuts pointing one chunk earlier than their parent segment; (2) the legacy `[SCENE]/[BROLL]/[BEAT]/grounder pass` design was still documented across `styles/niu-shu.md` §10 + §5.5b, `styles/first-person-pov.md` §10, `styles/_style_contract.md` (authority doc), `docs/HANDBOOK.md` core-concept sections — all of which feed the planner-writer prompt or the LLM's mental model; (3) Stage 0's VLM was forbidden from using franchise knowledge, so 17.7% of JJK0 segments had empty `characters` and cross-chunk character labels were inconsistent. Fixes: (a) `merge_segments` now shifts inner cuts; new `tmp/migrate_shot_boundaries.py` patches existing `visual_segments.json` files in-place via chunk-offset arithmetic (no API or ffmpeg cost); applied to JJK0 and SPL2 (~423 + 287 segments shifted, 0 misplaced after); (b) all style files and HANDBOOK rewritten for the `[ANCHOR]` contract; (c) Stage 0 prompt is now a `build_prompt(synopsis_text)` template — when a synopsis is attached via `--synopsis` (auto-detected from `movies/<slug>/synopsis.md` by the harness), the VLM is permitted to label characters from the cast reference but forbidden from inventing names outside it. New tests: 4 in `TestMergeSegments` for the offset shift, 7 in `TestBuildPrompt` and strategy-level for synopsis enrichment. 94/94 stage 0/2/contract tests pass.
